...
- **初回アイコン生成**: プロジェクト初期化・初回コミットの際には、必ず仮のアイコン画像（SVG等）を作成し、**`npm run icon`** (内部で `tauri icon` を実行) コマンドを使用してTauri用の各種アイコン（.ico, .png, .icns）を生成してください。
...
## 3. UIコンポーネントとテーマ・色の設計
- **推奨ライブラリ**: 可能であれば、**`shadcn-svelte`** や **`bits-ui`** を積極的に活用・導入していくことを推奨します。
- **レイアウト**: 狭いウィンドウサイズでも要素が重ならず視認性を保てるように配慮してください（レスポンシブなFlex/Gridレイアウトを推奨）。
- **テーマと色の定義**:
  - カラーパレットとして、あらかじめ **プライマリ (Primary)**、**セカンダリ (Secondary)**、**ターシャリ (Tertiary)** カラーの3つを定義・設定しておいてください。
  - テーマの設定や適用は、必ず **「ダーク / ライト / 端末（システム）設定に依存 / その他（カスタムテーマ等）」** のような形で定義・管理し、それに沿ってCSS変数等で切り替えられるように実装してください。

## 4. アーキテクチャとTauri v2の制約
