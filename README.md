...
- **整理と並び替え**:
  - **お気に入り (Favorites)**: 最大5件まで登録可能。タスクトレイの右クリックメニューから即座に起動できます。
  - **ピン留め**: 重要なリンクをリストの最上部に固定できます。
  - **検索ショートカット**: `Ctrl + F` キーで瞬時に検索窓へフォーカスできます。
...
インストーラーを作成する前に、**必ず一度アイコンの生成コマンドを実行してください。** これを行わないと、デフォルトのTauriアイコンが使用されます。

```bash
# 1. アイコンの生成 (初回のみ、またはSVGを変更した際)
npm run icon

# 2. インストーラーの作成 (MSI/NSIS)
npm run tauri build
```
